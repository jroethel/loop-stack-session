...
## 2026-09-13 - skills/loop-drive/SKILL.md + reference chain (read-edge demotion; gates Task 9 of the jrit-loop port plan)
- Harness snapshot: claude 2.1.269 probed 2026-09-13 (prior audit ran against 2.1.204); live probes via current-session tool schemas: background subagents with completion notifications, SendMessage, `isolation: worktree`, AskUserQuestion multiSelect all present; the Agent tool's model enum now lists fable, so Fable-never-a-worker stays an owner constraint the harness does not enforce. Ringer present (4 engines, `./ringer.py models --task-type` live, MODEL-NOTES and AMENDMENTS-PENDING exist); tracker.sh primitives (next-eligible, claim --reclaim, done --ran) verified in-script; baseline tests/run.sh 59/59 pre-edit.
- Directive: evaluate every mandatory-read edge for demotion to a conditional read; prefer demotion over deletion or rename (port-plan constraint, zero plan touch-up).
- Per-edge verdicts (the new "Conditional reads" block in SKILL.md is the single home of the triggers):
  - `config/routing/model-benchmarks.md`: DEMOTED - read when a routing decision arises (Step 2 assignment, runtime re-route at a gate, human-paced per-unit choice); never for Step 0 triage, a CHAT or DON'T BOTHER exit, or the existing-`_loop.md` pin review.
  - `references/ringer-substrate.md`: DEMOTED - read when at least one unit takes the ringer transport; never in degraded mode.
  - `references/native-orchestration.md`: DEMOTED - read when at least one unit takes the Agent-tool transport.
  - `references/fable-guidelines.md`: ALREADY CONDITIONAL (human-paced mode only); made explicit in the block; the duplicate "read it before drafting the run-book" clause dropped from the human-paced section (single-home).
  - `references/reviewer-conduct-contract.md`: DEMOTED - read at validator-dispatch time; the fail-closed stop on a missing file is unchanged.
  - `references/queue-runner.md`: NOT AN EDGE - no prose in skills/ or config/ reads it; it is a standalone pasteable operator artifact pinned by tests/gates/queue-runner.sh. Owner ruling 2026-09-13: keep in place, ships with the Task 9 copy.
  - `skills/loop-brainstorm/references/one-minute-test.md` (edge beyond the directive list): conditional by entry point - every fresh-plan Step 0 reads it, the existing-`_loop.md` entry skips it.
- queue-runner.md first content audit (deferred from the 2026-08-16 entry): all POLICY, zero deletions - the boundary-first list with its STOP class, the one-per-run stop rule, the evidence-gated done, and the reclaim path are all gate-test-pinned; the canonical copy-paste cycle kept (operator-facing template, a different consumer than the prompt prose); its embedded premise "the gate registry scrapes only skills/loop-*/SKILL.md" VERIFIED against scripts/gen-gate-registry.sh, which loops exactly that glob.
- Premises: one expired in config/routing/model-benchmarks.md, rewritten in place per the file's own maintenance rule - glm-5.3 graduated proven (3 first-try passes 2026-09-09, fields-json-metadata; claude-zai lane default rolled forward per MODEL-NOTES) and got a dated row; glm-5.2's "posterior depressed, read prior until amended" note replaced (posterior largely recovered by the 2026-08-21 catch-up amendments, code-feature 88% pass; the seven stm-nav amendments remain pending; superseded as lane default by glm-5.3).
- Deleted: one clause dedup (N-1), the human-paced "read it before drafting the run-book" trigger now single-homed in Conditional reads. Zero plumbing and zero choreography found this pass; net prose ADDS ~13 lines (the Conditional reads block), owner-directed, recorded as the demotion's cost.
- Subtraction test: tests/run.sh 59/59 post-edit; one real task - a fresh-context subagent invoked the edited SKILL.md on a one-off-question input, exited Step 0 with verdict CHAT, and per the skill's own text was required to read exactly one file beyond SKILL.md (one-minute-test.md), with all six chain references exempted and actually unread.
- Constraints re-confirmed: owner ruling 2026-09-13 inherits the 2026-08-16 register unchanged (/workflows off, portability/ringer-spine degraded mode, Fable-never-a-worker plus effort cap high, single-home-plus-pointers, claude-zai tie-break, 15-line BATCH threshold, MODEL-NOTES dual-repo receipts).
- Verdict: the chain is molted for the port - every file unchanged in location and name (zero plan touch-up), reads demoted to explicit triggers. This entry satisfies Task 9's gate condition (a loop-drive ledger entry dated after 2026-09-12).
