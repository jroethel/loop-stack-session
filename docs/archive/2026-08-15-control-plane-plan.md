...
- Produces:
  ...
  - `tracker.sh done <num> --receipt <text> [--ran <cmd>]` - the guarded completion path.
    ...
    - Otherwise validate `<text>`: it passes only if `printf '%s\n' "$text" | grep -qE '(exit( status)? 0( |$)|[1-9][0-9]* passed|(^| )0 failed|https?://[^ ]+)'` - i.e. a zero exit, a passing test summary, or an artifact URL (anchored so `10 failed` and `0 passed` are rejected; loop-review finding, fixed at the terminal gate). A **failing** exit (`exit 1`) and an evidence-free receipt are both rejected (exit 5, message `agent:done requires executed-check evidence of a PASSING run (exit 0 / N passed / 0 failed / artifact link)`).
